...
class BulkOperationsTest < Test::Unit::TestCase
  ...
  it_works_using_oauth1_and_oauth2 do |client, client_type|
    should "create multiple invoices at once using #{client_type}" do
      c1, c2 = nil, nil
      assert_true(
    ...
    can "create and update new records in bulk using #{client_type}" do
      c1, c2 = nil, nil
      assert_true(
          client.Contact.batch_save do
    ...
    can "return false from #batch_save if validation fails using #{client_type}" do
      assert_false(
          client.Contact.batch_save do
